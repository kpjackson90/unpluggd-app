...
const signup = (dispatch) => async(email, password, callback) => {
    try {
        const response = await api.post('/api/user/signup', { email, password, role: 'user' });
...
const createEventHost = (dispatch) => async(firstName, lastName, residence, occupation, bio, companyName, companyUrl, introVideo, companyBio, callback) => {
    try {
...
const updateCategories = (dispatch) => async (categories, isAdultContent, callback) => {
    try {
         await api.put('/api/user/update-profile', {
...
const updateHostSocial = (dispatch) => async (facebookUrl, twitterUrl, instagramUrl, youtubeUrl, twitch, callback) => {
    try {
