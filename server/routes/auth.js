...
router.post('/api/user/verify', async (req, res) => {
	...
	try {
		...
		return res.status(200).send({token, message: 'Email successfully verified'});
	} catch (err) {
		return res.status(400).send({error: err.message});
	}
});
...
router.get('/api/user/facebook/callback', passport.authenticate('facebook', {session: false}), (req, res) => {
	res.redirect('/');
});
